...
class CoordCube {
	...
	static {
		...
		//iterate through all possible corner orientations
		for (short i = 0; i < N_TWIST; i++) {
			a.setTwist(i);
			//iterate through all possible moves (cube face X face turn)
			//iterate all cube faces
			for (int j = 0; j < 6; j++) {
				//iterate all possible number of face turns (90, 180 or 270 (-90) degrees)
				for (int k = 0; k < 3; k++) {
	 ...
	 * 	Keeps for each combination of corners orientation and move the new corner orientation after executing the move. <br>
	 *  flip < 2048 in phase 1 <br>
	 *  flip = 0 in phase 2 */
	...
	static {
		CubieCube a = new CubieCube();
		//iterate through all possible permutations of edges FR, FL, BL and BR
	...
	static {
		CubieCube a = new CubieCube();
		//iterate through all possible permutations of corners URF, UFL, ULB, UBR, DFR and DLF
	...
	static {
		CubieCube a = new CubieCube();
		//iterate through all possible permutations of edges UR, UF, UL, UB, DR and DF 
		for (short i = 0; i < N_URtoDF; i++) {
			...
			for (int j = 0; j < 6; j++) {
				for (int k = 0; k < 3; k++) {
					...
					//note: table values are only valid for phase 2 moves! for phase 1 moves, casting to short is not possible.
				}
				//apply face turn one more time to restore cube
				a.edgeMultiply(CubieCube.moveCube[j]);
			}
